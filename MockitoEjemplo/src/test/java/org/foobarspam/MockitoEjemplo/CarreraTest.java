...
public class CarreraTest {
	...
	public void getNombreConductorTest(){
		...
		String nombre = "David";
		when(mockConductora.getNombre()).thenReturn(nombre);
		assertEquals(nombre, carrera.getNombreConductor());
	}
	
	@Test
	public void getModeloConductorTest(){
		Conductora mockedConductora = mock(Conductora.class);
		
		String modelo = "Toyota";
		carrera.setConductor(mockedConductora);
		when(mockedConductora.getModelo()).thenReturn(modelo);
		assertEquals(modelo, carrera.getModeloConductor());
	}
	
	@Test
	public void getMatriculaConductorTest(){
		Conductora jandro = mock(Conductora.class);
		
		carrera.setConductor(jandro);
		
		String matricula = "123456X";
		
		when(jandro.getMatricula()).thenReturn(matricula);
		assertEquals(matricula,carrera.getMatriculaConductor());
	}
	
	@Test
	public void getValoracionConductorTest(){
		Conductora deniz = mock(Conductora.class);
		
		carrera.setConductor(deniz);
		
		double valoracion = 3.0;
		
		when(deniz.getValoracion()).thenReturn(valoracion);
		assertEquals(valoracion,carrera.getValoracionConductor(),0.001);
